	...
	<div class="table-wrapper">
		<table>
			...
			<tr>
				...
				<th onclick="sortTable(1)">Mess</th>
				<th onclick="sortTable(2)">Meal Time</th>
				<th onclick="sortTable(3)">Date Occurred</th>
				<th onclick="sortTable(4)">Accepted At</th>
				<th onclick="sortTable(5)">Resolved At</th>
...
<<<<<<< HEAD
	...
	function sortTable(columnIndex) {
		...
		dataRows.sort((a, b) => {
			...
			if (columnIndex === 3 || columnIndex === 4 || columnIndex === 5) { // Date Occurred, Accepted At, Resolved At (dates)
				const dateA = aValue === 'N/A' ? 0 : new Date(aValue);
				const dateB = bValue === 'N/A' ? 0 : new Date(bValue);
				return isAscending ? dateB - dateA : dateA - dateB;
			} else { // Name, Mess, Meal Time, Category, Status (text)
				return isAscending ? bValue.localeCompare(aValue) : aValue.localeCompare(bValue);
			}
		});
